...
const messages: MessageCatalog = {
  en: {
    ...
    modulate: {
      ...
      presetMenu: (type: string) => `${type} preset menu`,
      soundParameters: "SOUND PARAMETERS",
      saveCurrentAs: "SAVE CURRENT AS…",
      saveCurrentAsType: (type: string) => `SAVE ${type} AS…`,
      noSavedPresets: "NO SAVED PRESETS",
      loadPresetNamed: (name: string) => `Load ${name}`,
      deletePresetNamed: (name: string) => `Delete ${name}`,
      json: "JSON",
      openPatchJson: "OPEN PATCH JSON…",
      savedPreset: "SAVED PRESET",
      savePreset: "SAVE PRESET",
      presetName: "PRESET NAME",
      presetNamePlaceholder: "e.g. Soft brass decay",
      presetNameHint: "SAVED IN THIS BROWSER",
      presetNameUnavailable: "USE A NEW, NON-EMPTY NAME.",
      save: "SAVE",
      cancel: "CANCEL",
  ...
  ja: {
    ...
    modulate: {
      ...
      presetPatch: "パッチ",
      presetOperator: "OP",
      presetParameters: "パラメータ",
      presetEnvelope: "エンベロープ",
      presetMenu: (type: string) => `${type}プリセットメニュー`,
      soundParameters: "サウンドパラメータ",
      saveCurrentAs: "現在値を別名で保存…",
      saveCurrentAsType: (type: string) => `${type}を別名で保存…`,
      noSavedPresets: "保存済みプリセットなし",
      loadPresetNamed: (name: string) => `${name}を読み込む`,
      deletePresetNamed: (name: string) => `${name}を削除`,
      json: "JSON",
      openPatchJson: "パッチJSONを開く…",
      savedPreset: "保存済みプリセット",
      savePreset: "プリセットを保存",
      presetName: "プリセット名",
      presetNamePlaceholder: "例: Soft brass decay",
      presetNameHint: "このブラウザに保存されます",
      presetNameUnavailable: "別の空でない名前を入力してください。",
      save: "保存",
      cancel: "キャンセル",
